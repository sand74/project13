...
from ....data.nan_filler import fill_nan

class _SAITS_CONSTANT_FC(nn.Module):
    def __init__(
        self,
        n_layers: int,
        n_steps: int,
        ...
        self.combining_weights_logits = nn.Parameter(torch.zeros(n_steps, n_features), requires_grad=True)
        stdv = 1. / math.sqrt(self.combining_weights_logits.size(1))
        self.combining_weights_logits.data.uniform_(-stdv, stdv)
    ...
    def _process(
        ...
        X = fill_nan(X, masks, self.nan_filler)
        ...
        first_DMSA_embedding = enc_output.clone()
        ...
        second_embedding = F.relu(self.reduce_dim_beta(enc_output))
        X_tilde_2 = self.reduce_dim_gamma(second_embedding)
        
        second_DMSA_embedding_prefinal = enc_output.clone()
        second_DMSA_embedding_final = second_embedding.clone()
        ...
        combining_weights = torch.sigmoid(self.combining_weights_logits)
        ...
        return (
            ...
            [first_DMSA_attn_weights, second_DMSA_attn_weights],
            [first_DMSA_embedding, second_DMSA_embedding_prefinal, second_DMSA_embedding_final]
        )
    ...
    def forward(
        ...
        results = {
            ...
            "first_DMSA_embeddings": first_DMSA_embedding,
            "second_DMSA_embeddings_prefinal": second_DMSA_embedding_prefinal,
            "second_DMSA_embeddings_final": second_DMSA_embedding_final,
        }
        if not training:
