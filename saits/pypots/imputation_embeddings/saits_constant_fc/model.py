...
from .modules import _SAITS_CONSTANT_FC
...
class SAITS_CONSTANT_FC(BaseNNImputer):
    ...
    def predict(
        ...
        first_DMSA_embedding_collector = []
        second_DMSA_embedding_prefinal_collector = []
        ...
        with torch.no_grad():
            for idx, data in enumerate(test_loader):
                ...
                if return_latent_vars:
                    ...
                    first_DMSA_embedding_collector.append(
                        results["first_DMSA_embeddings"].cpu().numpy()
                    )
                    second_DMSA_embedding_prefinal_collector.append(
                        results["second_DMSA_embeddings_prefinal"].cpu().numpy()
                    )
                    second_DMSA_embedding_final_collector.append(
                        results["second_DMSA_embeddings_final"].cpu().numpy()
                    )
        ...
        if return_latent_vars:
            latent_var_collector = {
                ...
                "second_DMSA_embeddings_prefinal": np.concatenate(
                    second_DMSA_embedding_prefinal_collector
                ),
                "second_DMSA_embeddings_final": np.concatenate(
                    second_DMSA_embedding_final_collector
                ),
                "first_DMSA_embeddings": np.concatenate(
                    first_DMSA_embedding_collector
                ),
                "combining_weights": torch.sigmoid(self.model.combining_weights_logits.detach()).cpu(),
                "first_DMSA_final_linear": {
                    "weight": self.model.reduce_dim_z.weight.detach().cpu(),
                    "bias": self.model.reduce_dim_z.bias.detach().cpu(),
                },
                "second_DMSA_prefinal_linear": {
                    "weight": self.model.reduce_dim_beta.weight.detach().cpu(),
                    "bias": self.model.reduce_dim_beta.bias.detach().cpu(),
                },
                "second_DMSA_final_linear": {
                    "weight": self.model.reduce_dim_gamma.weight.detach().cpu(),
                    "bias": self.model.reduce_dim_gamma.bias.detach().cpu()
                }
            }
