...
class BaseDataset(Dataset):
    ...
    def fill_nan(self, tensor: torch.tensor) -> torch.tensor:
        if self.nan_filler == "zeros":
            return torch.nan_to_num(tensor)
        if self.nan_filler == "last_value":
            filler = tensor[0].clone()
            for row in tensor:
                filler[~torch.isnan(row)] = row[~torch.isnan(row)]
                row[torch.isnan(row)] = filler[torch.isnan(row)]
            return tensor
        raise Exception("There is no fill nan strategy {}".format(self.nan_filler))
