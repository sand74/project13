...
class SAITS(BaseNNImputer):
    ...
    def fit(
        ...
        training_set = DatasetForSAITS(
            train_set, return_labels=False, file_type=file_type, nan_filler=self.nan_filler
        )
        ...
        if return_latent_vars:
            latent_var_collector = {
                ...
                "first_DMSA_final_linear": {
                    "weight": self.model.reduce_dim_z.weight.detach().cpu().numpy(),
                    "bias": self.model.reduce_dim_z.bias.detach().cpu().numpy(),
                },
                "second_DMSA_prefinal_linear": {
                    "weight": self.model.reduce_dim_beta.weight.detach().cpu().numpy(),
                    "bias": self.model.reduce_dim_beta.bias.detach().cpu().numpy(),
                },
                "second_DMSA_final_linear": {
                    "weight": self.model.reduce_dim_gamma.weight.detach().cpu().numpy(),
                    "bias": self.model.reduce_dim_gamma.bias.detach().cpu().numpy()
                }
            }
